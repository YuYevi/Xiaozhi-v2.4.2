...
/**
 * @file shake_main.c
 * @brief BMI270 shake detection example for ESP32
 *
 * This example demonstrates how to initialize the BMI270 sensor, configure it for
 * shake detection, and handle interrupts using ESP32 GPIO. The code includes
 ...
 *
 * Maps the shake interrupt to INT1 pin and configures interrupt pin properties.
 *
 * @param[in] bmi2_dev Pointer to BMI2 device structure
 *
 * @return BMI2 API execution status
...
/**
 * @brief Enable shake detection interrupt and handle detection events
 *
 * This function enables the shake detection feature, configures interrupts, and enters a loop
 * to handle shake events. It detects both slight and heavy shakes on X, Y, Z axes.
 *
 * @param[in] bmi2_dev Pointer to BMI2 device structure
 *
 * @return BMI2 API execution status
...
static int8_t bmi270_enable_shake_int(bmi270_handle_t bmi2_dev)
{
    ...
    /* Enable shake detection feature */
    rslt = bmi270_enable_toy_shake(bmi2_dev, BMI2_ENABLE);
    bmi2_error_codes_print_result(rslt);
    ESP_LOGI("MAIN", "Shake feature enabled, result: %d", rslt);

    if (rslt == BMI2_OK) {
        ESP_LOGI("MAIN", "Move the sensor to get shake interrupt...");
        ...
        while (rslt == BMI2_OK) {
            if (interrupt_status) {
                ...
                if (int_status & BMI270_TOY_INT_SHAKE_MASK) {
                    uint8_t data;
                    rslt = bmi2_get_regs(0x1f, &data, 1, bmi2_dev);

                    /* Check for slight shake */
                    if (data & 0x08) {
                        ESP_LOGI("MAIN", "Slight shake on: ");
                        if (data & 0x01) {
                            ESP_LOGI("MAIN", "X axis");
                        }
                        if (data & 0x02) {
                            ESP_LOGI("MAIN", "Y axis");
                        }
                        if (data & 0x04) {
                            ESP_LOGI("MAIN", "Z axis");
                        }
                    }
                    /* Check for heavy shake */
                    if (data & 0x80) {
                        ESP_LOGI("MAIN", "Heavy shake on: ");
                        if (data & 0x10) {
                            ESP_LOGI("MAIN", "X axis");
                        }
                        if (data & 0x20) {
 ...
 *
 * This function initializes GPIO, I2C, and BMI270, then enables shake detection.
 * It also handles cleanup after the detection loop finishes.
 */
void app_main(void)
{
    ESP_LOGI("MAIN", "=== BMI270 Shake Detection Configuration ===");
#ifdef CONFIG_BOARD_ESP_SPOT_C5
    ...
    int8_t rslt = bmi270_enable_shake_int(bmi_handle);
    if (rslt != BMI2_OK) {
        ESP_LOGE("MAIN", "BMI270 enable shake interrupt failed");
        bmi2_error_codes_print_result(rslt);
    }
    ...
    ESP_LOGI("MAIN", "BMI270 shake interrupt example finished.");
}
