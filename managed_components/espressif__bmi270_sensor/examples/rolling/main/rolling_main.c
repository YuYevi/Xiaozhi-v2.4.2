...
/**
 * @file rolling_main.c
 * @brief BMI270 rolling detection example for ESP32
 *
 * This example demonstrates how to initialize the BMI270 sensor, configure it for
 * rolling detection, and handle interrupts using ESP32 GPIO. The code includes
 * I2C initialization, sensor configuration, interrupt service routines, and main
 * application logic to detect device orientation changes.
 */
 ...
 *
 * Maps the rolling interrupt to INT1 pin and configures interrupt pin properties.
 *
 * @param[in] bmi2_dev Pointer to BMI2 device structure
 *
 * @return BMI2 API execution status
...
{
    int8_t rslt;
    uint8_t data = BMI270_TOY_INT_GI_INS1_ROLLING_MASK;
    struct bmi2_int_pin_config pin_config = { 0 };

    /* Map rolling interrupt to INT1 pin */
...
/**
 * @brief Enable rolling detection interrupt and handle detection events
 *
 * This function enables the rolling detection feature, configures interrupts, and enters a loop
 * to handle rolling events. When the device orientation changes, it displays the current
 * orientation direction (X/Y/Z axis up or down).
 *
 * @param[in] bmi2_dev Pointer to BMI2 device structure
 *
 * @return BMI2 API execution status
 ...
 *      - Other values: Failure
 */
static int8_t bmi270_toy_enable_rolling_int(bmi270_handle_t bmi2_dev)
{
    int8_t rslt;
    ...
    /* Enable rolling detection feature */
    ...
    if (rslt == BMI2_OK) {
        ESP_LOGI("MAIN", "Move the sensor to get rolling interrupt...");
        ...
        while (rslt == BMI2_OK) {
            if (interrupt_status) {
                ...
                if (int_status & BMI270_TOY_INT_GI_INS1_ROLLING_MASK) {
                    ...
                    switch (data) {
                    case 1:
                        ESP_LOGI("MAIN", "🔄 Rolling: X upside");
                        ESP_LOGI("MAIN", "   ↑");
                        ESP_LOGI("MAIN", "   |");
                        break;
                    case 2:
                        ESP_LOGI("MAIN", "🔄 Rolling: X downside");
                        ESP_LOGI("MAIN", "   |");
                        ESP_LOGI("MAIN", "   ↓");
                        break;
                    case 3:
                        ESP_LOGI("MAIN", "🔄 Rolling: Y upside");
                        ESP_LOGI("MAIN", "   ←——");
                        break;
                    case 4:
                        ESP_LOGI("MAIN", "🔄 Rolling: Y downside");
                        ESP_LOGI("MAIN", "   ——→");
                        break;
                    case 5:
                        ESP_LOGI("MAIN", "🔄 Rolling: Z upside");
                        ESP_LOGI("MAIN", "   ↻");
                        break;
                    case 6:
                        ESP_LOGI("MAIN", "🔄 Rolling: Z downside");
                        ESP_LOGI("MAIN", "   ↺");
                        break;
                    default:
                        ESP_LOGI("MAIN", "🔄 Rolling: Unknown direction");
                        ESP_LOGI("MAIN", "   ?");
                        break;
                    }
                }
 ...
 *
 * This function initializes GPIO, I2C, and BMI270, then enables rolling detection.
 * It also handles cleanup after the detection loop finishes.
 */
void app_main(void)
{
    ESP_LOGI("MAIN", "=== BMI270 TOY Rolling Detection Configuration ===");
#ifdef CONFIG_BOARD_ESP_SPOT_C5
    ...
    /* Configure GPIO for interrupt (INT1 for rolling detection) */
    ...
    int8_t rslt = bmi270_toy_enable_rolling_int(bmi_handle);
    if (rslt != BMI2_OK) {
        ESP_LOGE("MAIN", "BMI270 TOY enable rolling interrupt failed");
        bmi2_error_codes_print_result(rslt);
    }
    ...
    ESP_LOGI("MAIN", "BMI270 TOY rolling detection interrupt example finished.");
}
