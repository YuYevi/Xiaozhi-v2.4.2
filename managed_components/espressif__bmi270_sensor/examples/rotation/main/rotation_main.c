...
/**
 * @file rotation_main.c
 * @brief BMI270 rotation detection example for ESP32
 *
 * This example demonstrates how to initialize the BMI270 sensor, configure it for
 * rotation detection, and process rotation data. The code includes I2C initialization,
 * sensor configuration, and rotation angle reading from the sensor.
 */
...
#define UE_SW_I2C               1
#elif defined(CONFIG_BOARD_M5STACK_ATOMS3)
#define I2C_INT_IO              16
#define I2C_MASTER_SCL_IO       0
#define I2C_MASTER_SDA_IO       45
#define UE_SW_I2C               1
#elif defined(CONFIG_BOARD_ESP_VOCAT_S3)
...
/**
 * @brief Adjust rotation detection parameters
 *
 * This function modifies the static threshold for rotation detection.
 * Lower threshold (0x0014) is more sensitive for table placement.
 * Higher threshold (0x0080) is more robust for in-hand usage.
 *
 * @param[in] bmi2_dev Pointer to BMI2 device structure
 */
static void adjust_rotation_parameter(bmi270_handle_t bmi2_dev)
{
    uint8_t data[2], page;
    ...
    page = 1;
    bmi2_set_regs(0x2f, &page, 1, bmi2_dev);

    /* Static threshold configuration - sensitive for table placement */
    uint16_t static_threshold = 0x0014;

    /* For robust config (e.g., in hand), use: static_threshold = 0x0080 */
...
/**
 * @brief Enable rotation detection and read rotation angle continuously
 *
 * This function enables the rotation detection feature and enters a loop to continuously
 * read rotation angle data. The rotation angle is displayed with visual indicators showing
 ...
 *      - Other values: Failure
 */
static int8_t bmi270_toy_enable_rotation_int(bmi270_handle_t bmi2_dev)
{
    int8_t rslt;
    ...
    adjust_rotation_parameter(bmi2_dev);

    /* Enable rotation detection feature */
    rslt = bmi270_enable_toy_rotation_angle(bmi2_dev, BMI2_ENABLE);
    bmi2_error_codes_print_result(rslt);
    ESP_LOGI("MAIN", "Rotation feature enabled, result: %d", rslt);

    vTaskDelay(pdMS_TO_TICKS(100));

    if (rslt == BMI2_OK) {
        ESP_LOGI("MAIN", "Rotate the sensor to get rotation angle...");

        while (rslt == BMI2_OK) {
            float angle;
            uint8_t status;

            rslt = bmi270_get_toy_rotation_angle(bmi2_dev, &status, &angle);
            bmi2_error_codes_print_result(rslt);

            if (status != BMI2_OK) {
                ESP_LOGI("MAIN", "Please keep static for a while...");
            } else {
                /* Visual representation of rotation angle */
                if (angle > 0) {
                    ESP_LOGI("MAIN", "   ↺ +%.2f°", angle);
                } else if (angle < 0) {
                    ESP_LOGI("MAIN", "   ↻ %.2f°", angle);
                } else {
                    ESP_LOGI("MAIN", "   ⊙ 0°");
 ...
 *
 * This function initializes I2C and BMI270, then enables rotation detection.
 * It also handles cleanup after the detection loop finishes.
 */
void app_main(void)
{
    ESP_LOGI("MAIN", "=== BMI270 TOY Rotation Angle Detection Configuration ===");
#ifdef CONFIG_BOARD_ESP_SPOT_C5
    ESP_LOGI("MAIN", "Selected Board: ESP SPOT C5");
#elif defined(CONFIG_BOARD_ESP_SPOT_S3)
    ESP_LOGI("MAIN", "Selected Board: ESP SPOT S3");
#elif defined(CONFIG_BOARD_M5STACK_ATOMS3)
    ESP_LOGI("MAIN", "Selected Board: M5Stack AtomS3");
#elif defined(CONFIG_BOARD_ESP_VOCAT_S3)
    ESP_LOGI("MAIN", "Selected Board: ESP-VoCat S3");
#elif defined(CONFIG_BOARD_CUSTOM)
    ESP_LOGI("MAIN", "Selected Board: Other Boards (Custom)");
#else
    ESP_LOGI("MAIN", "Selected Board: Default (VOCAT_S3)");
#endif
    ...
    int8_t rslt = bmi270_toy_enable_rotation_int(bmi_handle);
    if (rslt != BMI2_OK) {
        ESP_LOGE("MAIN", "BMI270 TOY enable rotation interrupt failed");
        bmi2_error_codes_print_result(rslt);
    }
    ...
    ESP_LOGI("MAIN", "BMI270 TOY rotation angle detection example finished.");
}
