...
/**
 * @file push_in_air_main.c
 * @brief BMI270 push-in-air detection example for ESP32
 *
 * This example demonstrates how to initialize the BMI270 sensor, configure it for
 * push-in-air detection, and handle interrupts using ESP32 GPIO. The code includes
 ...
 *
 * Maps the push interrupt to INT1 pin and configures interrupt pin properties.
 *
 * @param[in] bmi2_dev Pointer to BMI2 device structure
 *
 * @return BMI2 API execution status
...
static int8_t set_feature_interrupt(bmi270_handle_t bmi2_dev)
{
    ...
    /* Map push interrupt to INT1 pin */
 ...
 * to handle push detection events. When a push is detected, it waits 3 seconds before
...
void app_main(void)
{
    ESP_LOGI("MAIN", "=== BMI270 TOY Push in Air Detection Configuration ===");
#ifdef CONFIG_BOARD_ESP_SPOT_C5
    ...
    ESP_LOGI("MAIN", "BMI270 TOY push in air interrupt example finished.");
}
